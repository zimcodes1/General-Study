...
import { UserX, Mail, ArrowRight } from 'lucide-react';
...
export default function UserManagement({
  ...
  const displayed = limit !== undefined ? users.slice(0, limit) : users;
        ...
        <table className="w-full">
          <thead className="bg-surface-container">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-semibold text-on-surface-variant uppercase tracking-wider">Email</th>
              <th className="px-6 py-3 text-left text-xs font-semibold text-on-surface-variant uppercase tracking-wider">Department</th>
              <th className="px-6 py-3 text-left text-xs font-semibold text-on-surface-variant uppercase tracking-wider">Level</th>
          ...
          <tbody className="divide-y divide-outline-variant/10">
            {displayed.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-10 text-center text-on-surface-variant text-sm">No users found</td>
              </tr>
            ) : (
              displayed.map((user) => (
                <tr key={user.id} className="hover:bg-surface-container/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-semibold uppercase ${user.status === 'active'
